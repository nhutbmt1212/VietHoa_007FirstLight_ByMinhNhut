...
BATCH_SIZE     = 16     # Tăng từ 8 lên 16 — mỗi lần gọi AI dịch nhiều hơn
CONTEXT_WINDOW = 12     # Giảm từ 25 xuống 12 — đủ nhớ ngữ cảnh, ít token hơn
TEMPERATURE    = 0.05
TIMEOUT        = 360
...
▌ BẢNG ĐẠI TỪ THEO NHÂN VẬT
  ...
  Hostile, Mercenary, Bullthorp, Pirate (kẻ thù cấp thấp):
    • Tự xưng              → "tao"
    • Gọi Bond 1 mình      → "mày" (KHÔNG dùng "ngươi" — ngươi chỉ dành cho villain cấp cao)
...
━━━ QUY TẮC KỸ THUẬT ━━━
• GIỮ NGUYÊN TUYỆT ĐỐI [Tag] stage directions: [Laughs] [Sighs] [Coughs] [Groans] [Whispers] [In Serbian] v.v.
  → KHÔNG dịch thành [Cười] [Thở dài] hay bất kỳ ngôn ngữ nào khác
  → KHÔNG xóa tag
  → KHÔNG thêm tag mới
  → Ví dụ ĐÚNG: "[Laughs] Không ai thực sự biến mất cả."
  → Ví dụ SAI:  "[Cười] Không ai thực sự biến mất cả." ← SAI
  → Ví dụ SAI:  "Không ai thực sự biến mất cả." ← SAI (xóa tag)
• GIỮ NGUYÊN: {0} {1} {2} placeholders — không xóa, không dịch, không di chuyển
• GIỮ NGUYÊN: <i>text</i> <b>text</b> — chỉ dịch phần text bên trong tag HTML
• GIỮ NGUYÊN: \n ký tự xuống dòng
• GIỮ NGUYÊN: Dấu nháy kép `"` quanh tên/ký hiệu như "M", "007" — KHÔNG đổi thành []
  → Câu gốc: "M" is that a codename?
  → ĐÚNG: "M" là mật danh sao?
  → SAI:  [M] là mật danh sao?  ← TUYỆT ĐỐI KHÔNG làm vậy
• KHÔNG thêm bất kỳ tag {} nào không có trong bản gốc
• KHÔNG thêm dấu ngoặc kép bao ngoài câu dịch
...
def fix_quoted_brackets(orig: str, trans: str) -> str:
    """
    Nếu bản gốc có "X" (dấu nháy kép quanh chữ) mà bản dịch đổi thành [X],
    restore lại thành "X".
    Ví dụ: EN="M" is that a codename? → VI=[M] là mật danh sao? → fix → "M" là mật danh sao?
    """
    # Tìm tất cả "WORD" trong bản gốc
    quoted = re.findall(r'"([^"]{1,10})"', orig)
    for word in quoted:
        # Nếu VI có [word] thay vì "word"
        trans = re.sub(
            r'\[' + re.escape(word) + r'\]',
            f'"{word}"',
            trans
...
TUYỆT ĐỐI KHÔNG xóa hoặc dịch các thẻ trong ngoặc vuông như [Laughs], [In Montenegrin] v.v. Bắt buộc phải giữ nguyên chúng ở đầu câu!"""
    ...
    for (ek, sk, spk, orig), trans in zip(items, raw_results):
        ...
        # Post-process nhân xưng + spacing + quoted brackets
        trans = post_process(trans, spk, orig)
        final.append(trans)
        ...
        for key, entry in out_data.items():
            ...
            for sk, txt in entry.get("segments", {}).items():
                if not isinstance(txt, str):
                    continue
                new = post_process(txt, spk, txt)  # orig=txt vì không có EN ở fix-only
                if new != txt:
